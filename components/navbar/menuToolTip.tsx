                    ...
                    {list.map((item,index)=>{
                        ...
                        return(
                            <Accordion key={`${item.text}-${index}`}  showDivider={false} className="w-fit p-0" isCompact  hideIndicator itemClasses={{base: "py-0 w-full",title: "w-fit text-xs 2xl:text-sm text-forground hover:text-cmr-gray-500 transition-all ease-out duration-200 "}}>
                                <AccordionItem aria-label="Accordion 2" onPress={() => rotation(1)} title={<div className="flex gap-2 items-center"><Link href={item.link!} onClick={OnClose}>{item.text}</Link><FaAngleDown className={`inline-block ml-1 rotate-${rotate[1]} transition-all ease-out duration-200`}/></div> }>
